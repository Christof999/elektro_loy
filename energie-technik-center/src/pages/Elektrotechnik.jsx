...
/* ── Individuelle Illustrationen pro Leistung ── */
...
function SmartHomeIllustration() {
  return (
    <svg viewBox="0 0 220 110" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      ...
      {/* Smarthome-Devices links/rechts */}
      {/* Thermostat */}
  ...
  return (
    <>
      ...
      <section style={{
        ...
        background: 'linear-gradient(160deg, #0b1929 0%, #0f172a 55%, #1e1b4b 100%)',
        position: 'relative', overflow: 'hidden',
      }}>
        {/* Hintergrundszene: Schaltkreis nahtlos */}
        <ElektroSzene />
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
          <svg style={{ width: '100%', height: '100%', opacity: 0.03 }} aria-hidden="true">
            <defs><pattern id="eg2" x="0" y="0" width="28" height="28" patternUnits="userSpaceOnUse"><circle cx="1.5" cy="1.5" r="1.5" fill="white"/></pattern></defs>
            <rect width="100%" height="100%" fill="url(#eg2)"/>
          </svg>
        </div>
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          <div className="badge badge-dark" style={{ marginBottom: '0.9rem' }}>Bereich</div>
          <h1 style={{
            fontSize: 'clamp(2rem, 4.5vw, 3.2rem)', fontWeight: 900,
            color: '#fff', lineHeight: 1.12, marginBottom: '1rem', letterSpacing: '-0.02em', maxWidth: 540,
          }}>
            Elektrotechnik
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.68)', fontSize: '1.05rem', lineHeight: 1.8, maxWidth: 500, marginBottom: '2rem' }}>
      ...
      {/* ── Leistungen mit individuellen Illustrationen ── */}
                  ...
                  <div style={{ padding: '1.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.6rem' }}>
                      <div style={{
                        ...
                        <ItemIcon size={18} color="var(--accent)" />
                      </div>
                    ...
                    <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.32rem' }}>
                      {items.map(i => (
                        <li key={i} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', fontSize: '0.83rem', color: 'var(--text-secondary)' }}>
                          <CheckCircle size={12} color="var(--accent)" style={{ flexShrink: 0 }} /> {i}
                        </li>
                      ))}
                    </ul>
                  </div>
            ...
            <div className="reveal">
              ...
              {[
                ...
                <div key={title} style={{ display: 'flex', gap: '0.85rem', marginBottom: '1rem', alignItems: 'flex-start' }}>
                  <CheckCircle size={17} color="var(--accent)" style={{ marginTop: 2, flexShrink: 0 }} />
                  <div>
                    <div style={{ fontWeight: 600, fontSize: '0.92rem', marginBottom: '0.15rem' }}>{title}</div>
                    <div style={{ color: 'var(--text-secondary)', fontSize: '0.86rem', lineHeight: 1.6 }}>{text}</div>
                  </div>
                </div>
              ))}
            </div>
            {/* Schaltkreis-Grafik rechts – freistehend ohne Box */}
            <div className="reveal">
              <SchaltkreisGrafik />
            </div>
          </div>
      ...
      {/* ── CTA ── */}
      <section style={{ background: 'linear-gradient(135deg, #0b1929, #1e1b4b)', padding: '3.5rem 0' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.6)', marginBottom: '1.5rem' }}>Wir erstellen Ihnen ein kostenloses Angebot.</p>
...
/* ── Hintergrundszene: animierter Schaltkreis ── */
  ...
  return (
    <svg viewBox="0 0 900 380" preserveAspectRatio="xMidYMid slice"
      ...
      <style>{`
        ...
        @keyframes el-wave{0%{stroke-dashoffset:80}100%{stroke-dashoffset:0}}
        ...
        .el-wave{stroke-dasharray:80;animation:el-wave 1.3s linear infinite}
      `}</style>
      {/* Circuit board Leiterbahnen */}
      ...
      {/* Blitz in der Mitte */}
      <path className="el-bolt" d="M456 130 L448 152 L455 152 L447 174 L465 148 L457 148Z"
        fill="#fbbf24"/>
      {/* Sinus-Wellen */}
      <path className="el-wave" d="M80 330 Q100 315 120 330 Q140 345 160 330 Q180 315 200 330 Q220 345 240 330 Q260 315 280 330 Q300 345 320 330"
      ...
      {/* Kondensatoren */}
      <line x1="446" y1="240" x2="446" y2="252" stroke="#60a5fa" strokeWidth="2" strokeLinecap="round" opacity="0.7"/>
      <line x1="454" y1="240" x2="454" y2="252" stroke="#60a5fa" strokeWidth="2" strokeLinecap="round" opacity="0.7"/>
    </svg>
  );
}

/* ── Freistehende Schaltkreis-Grafik für "Warum wir"-Sektion ── */
